...
function stringify(node, builder) {
	...
	if (node.nodes.length) {
		node.nodes.forEach((root) => {
			...
			const mdBlockquoteMarker = root.raws.mdBlockquoteMarker;
			let blockquoteMarkerReplacer;
			if (mdBlockquoteMarker) {
				const baseMarker = mdBlockquoteMarker.base || "> ";
				const markers = mdBlockquoteMarker.markers || [];
				...
				let needMark = true;
				blockquoteMarkerReplacer = (origStr) => {
					if (!origStr) {
						return origStr;
					}
