...
function extractStyles(source, opts) {
	...
	function processHtml(startNode, endNode, parent) {
		if (startNode === endNode && isHtmlComment(startNode.value || "")) {
			// comment
			return;
		}
		const startIndex = startNode.position.start.offset;

		addStyle(
			{
				startIndex,
				isMarkdown: false,
				content: source.slice(startIndex, endNode.position.end.offset),
				lang: "html",
			},
			parent
		);
		...
		if (node.type === "code") {
			...
			if (htmlInMd) {
				...
				if (!next || next.type !== "html") {
					...
					processHtml(startNode, node, parent);
				}
			}
			if (node.value) {
